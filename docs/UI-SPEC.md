...
`ModalBase` titled `The Latent Lounge`, opened by the header `Lounge` tile, with two tabs over one shared bet control.
**The bet control**: a number box and a slider showing the same figure (the slider runs over the bank, not over a percentage, so the number under the thumb is the number that is staked), plus `Min · Half · Double · 25% · 50% · All in`. Nothing in here is priced in seconds of income and there is no cooldown to wait out; below `BET_MIN` credits the control is dead rather than quietly rounding up.
**Wheel**: the KSampler node mock, `seed` (an odometer that scrambles then settles, instant under reduced motion), `control_after_generate` (`randomize`, or `fixed` once the sampler is hot) and `denoise` (the pity meter as three pips), with the seven segments scrolling past a fixed marker, then the odds table and the pot.
**Coin flip**: one coin that turns for 0.9 s and lands on `You` or the Comfy mark, with both sides' chances and payouts in a two-row table under it.
