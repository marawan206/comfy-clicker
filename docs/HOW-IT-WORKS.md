...
The rest of the level is UI: a `LevelChip` in the header with a 3 px electric bar, a `LevelUpBanner` that merges two level-ups into `Level 4 to 6` (it reads `unlocked`, the model ids; the event also carries `hardware`, the unit ids), confetti plus an electric flash from `FxCanvas`, a first section in the Stats modal with the XP breakdown, and a `level-locked` state on `ModelCard` that beats `api-locked` and `needs-setup`. A level-locked unit shows in the store as locked with `canBuy`'s reason, through the same `nonCreditLock` path as any other store lock.
