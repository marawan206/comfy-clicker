...
- **A player level derived from lifetime stats**, which gates the model list (SD 1.5 at level 1, FLUX.1 dev at 4, video at 5 and up, API models at 7 and up), and a **Seed Roulette** with a printed odds table, a three-minute cooldown and payouts that deliberately cannot touch XP or the leaderboard.
...
### Seed Roulette (`src/game/gamble.ts`, `src/data/gamble.ts`)

A KSampler-themed spin behind a header tile, gated at `SPIN_MIN_LEVEL = 2`. In `GAMBLE_OUTCOMES` the `weight` **is** the probability and the table sums to exactly 1, so `weightedPick` draws from it directly and the expected value is the plain weighted mean.

| id | label | mult | p | pays |
|---|---|---|---|---|
| `nan` | NaN latent | ×0 | 0.340 | 0 |
| `half` | Half denoised | ×0.5 | 0.300 | 0.150 |
| `same` | Same seed, same image | ×1 | 0.172 | 0.172 |
| `clean` | Clean sample | ×2 | 0.120 | 0.240 |
| `batch` | Batch of four | ×4 | 0.050 | 0.200 |
| `golden` | Golden seed | ×10 | 0.015 | 0.150 |
| `s42` | Seed 42 | ×42 | 0.003 | 0.126 |

`spinEv()` is therefore **1.038**: something comes back 66 % of the time and 18.8 % of spins turn a profit. That is enough to make a three-minute cooldown interesting and far too little to be an income source, which `gamble.test.ts` pins outright with the invariant `(3600 / SPIN_COOLDOWN_MS × 1000) × (spinEv() − 1) × SPIN_MAX_SECS ≤ 0.07 × 3600`: even spinning the maximum wager every three minutes for an hour, the roulette adds at most 7 % of one hour's income.

Three rules keep it unspammable. The cooldown (`SPIN_COOLDOWN_MS = 180 000`) is a single timestamp in the save, so closing the tab, reloading or sleeping for eight hours all yield exactly one ready spin and never a backlog. The wager is priced in seconds of income, so it scales with the rig instead of with patience: `min = max(SPIN_MIN_WAGER (50), round(SPIN_MIN_SECS (30) × cps))`, `max = min(credits, max(min, round(SPIN_MAX_SECS (300) × cps)))`. And `spin` is never reachable from the click path or a hotkey.

On top of that: one free spin per UTC day with a house stake of `max(SPIN_FREE_MIN (200), round(SPIN_FREE_SECS (120) × cps))`, which deducts nothing and neither starts nor consumes the paid cooldown. A pity meter, where `dryStreak` counts consecutive `nan` results and at `SPIN_PITY_DRY = 3` the next losing draw is converted once into the cheapest winning segment (the draw still happens, so the rng sequence is identical either way). A hot sampler, where `winStreak` counts consecutive ×2-or-better results and at `SPIN_HOT_STREAK = 3` the widget flips to `control_after_generate: fixed` and payouts carry `SPIN_HOT_MULT = 1.5` until anything under ×2 breaks it (a `nan`, a `half` or even a `same`). And a pot, fed `round(SPIN_POT_FRACTION (0.02) × wager)` by every paid spin, which `s42` takes whole on top of its own payout, this spin's contribution included.

**The documented exception**: a payout moves `state.credits` and nothing else. `lifetimeCredits` feeds XP and therefore the player level, and `seasonCredits` feeds prestige and the leaderboard, so crediting either here would turn the roulette into an XP farm and let a lucky seed buy a rank. It is the one credit path in the engine that does not go through `addCredits`, it looks like a missing line, and a test pins that both counters stay put. The spin does not call `noteSpend` either: a bank that lands on zero because a seed ate the wager is not "Out Of Credits, Not Ideas".

Achievements: visible `seed-first` "Random Seed" and `seed-100` "Degenerate Sampler"; hidden `seed-42-hit` "It Was Always 42" (`jackpot42`), `seed-nan-3` "NaN, NaN, NaN" (`nanStreak3`), `seed-hot` "Control After Generate: Fixed" (`hotSeed`). `SeedModal` prints the whole odds table above the fold and the footer never hides the edge; `FxCanvas` throws confetti at ×10 and up, and only the ×42 raises a toast, because everything smaller is already on the reel in front of you.
...
| `jackpot42`, `nanStreak3`, `hotSeed` | Seed Roulette: the ×42, three NaNs in a row, three wins in a row | "It Was Always 42", "NaN, NaN, NaN", "Control After Generate: Fixed". |
...
Badges are counts with `BADGE_CAP = 9` (`9+` above it): Map counts Graph nodes that are available and affordable right now, Hub the runs accumulated in `localStorage['comfy-clicker:hub-unseen']` since the last `/hub` visit, Board a rank read from a cache the leaderboard page writes with `RANK_TTL_MS = 60 min` (a rank older than that is a lie, so the badge drops rather than misleads), Stats the achievements earned since `comfy-clicker:stats-seen`, Seed a bare dot while the free spin is waiting, Help a dot until the tour has run.
