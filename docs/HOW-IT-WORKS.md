...
- **A player level earned by doing things** (posting, contracts, the Graph, achievements, every new card, on top of the credits earned), which gates both the hardware ladder (a 3060 at level 2, a 4090 at 4, a B200 at 10, regions from 12) and the model list (SD 1.5 at level 1, FLUX.1 dev at 4, video at 5 and up, API models at 7 and up), and a **Latent Lounge** (a wheel and a coin flip) with printed odds tables, no cooldown and payouts that deliberately cannot touch XP or the leaderboard.
- **The engine is pure TypeScript with no React**, driven by a catalog of data tables, unit-tested (28 test files under `src/game/__tests__`, 56 across the repo) and paced by a simulator that the tests assert against.
...
Every field added since launch carries a schema fallback, so a blob written before it still loads: the eleven new `stats` counters, `settings.autosave` (true), the whole `gamble` block, and on a post `ratioed`, `mismatchedTags`, `nearViral` and a signed `followersGained`. Four hydrate rules are worth knowing. A save with no `stats.xpBy` at all gets one seeded by `legacyXp` from the counters it already carries, under today's weights (posts at a flat `LEGACY_POST_XP = 10` each, since the models they ran were never recorded); that can add up to a level above the watermark, and the next `settleLevelUps` pays and announces it on purpose, because the patch gave them a level. A save with no `stats.levelSeen` is stamped with `playerLevel(state)` after that seeding, so a veteran gets the level their stats already earned with no back-pay and no toast storm. A `stats.clickLockUntil` carried by an older blob collapses to 0, because the cadence lockout it was serving no longer exists. A citizen run scheduled by a clock that ran ahead is pulled back to `now + CITIZEN_TREND_MS`.
...
**The combo** (`src/game/combo.ts`). Accepted clicks no more than `COMBO_GAP_MS = 400` ms apart form a streak, and the streak's length picks a tier from `COMBO_TIERS`: from 10 clicks every click pays ×1.25, from 25 ×1.5, from 50 ×2, from 100 ×3, until the gap passes. The multiplier sits on top of the lucky seed (`value = clickValue × lucky × mult`), the `click` event carries `combo` and `mult`, and the streak itself is session bookkeeping in a `WeakMap`, not save data; the one persisted figure is `stats.bestCombo`, behind the visible achievements "Sampler on Fire" (50) and "CFG Overdrive" (100). The pill on the button's shoulder reads the event rather than counting for itself, shows `combo ×N` with a `×2 pay` badge, turns amber at 25, electric at 50 and pink at 100, and the click that reaches a tier pops it, throws a bigger burst, pitches the cash cue up a step per tier and, from double pay, washes the screen in the tier's colour. A refused click never reaches the combo, so the cap neither counts nor breaks a streak.

### The auto-clicker guard (`src/game/clickGuard.ts`, `src/lib/input.ts`)

Two layers, cheapest first, and neither of them ever stops the rack: hardware income is untouched, nothing is persisted, and the copy is a joke rather than an accusation.
...
2. **A hard rate cap**, `CLICK_CAP_PER_SEC = 20` accepted clicks over a trailing second, and it is the only cap. A refused click pays nothing, does not bump `totalClicks`, does not shave job time and emits no `click` event; it is answered with a `clickBlocked{reason: 'rate', until}` event, where `until` is the moment the oldest counted click leaves the window and the button pays again. It is explicitly **not** a strike: a burst is not cheating, it just does not count, and a fast hand is allowed. The first refusal raises `flags.clickGuard` behind the hidden achievement "Rate Limited", and that is the only thing a refusal writes.

There used to be a third layer, a cadence detector (mean and coefficient of variation over the last 24 attempted intervals) with a 10 / 30 / 60 s lockout ladder. It kept flagging fast humans as scripters, so it is gone: no lockout, no strikes, no decay. `stats.clickLockUntil / clickStrikes / clickStrikeAt` remain in the save as legacy fields (never written; `hydrate` collapses a stored lockout to 0) so older blobs keep their shape. The counted timestamps live in a `WeakMap` side table (the same pattern as the tick memo, because they are session bookkeeping). The "Batch Size: Yes" egg (100 clicks in 10 s) stays reachable: 10/s is half the cap, and a test pins it.

A refused click is silent rather than misleading: `clickBlocked` suppresses every piece of click feedback (no float, no burst, no ripple, no squash), the combo pill reads `MAX` for 600 ms, and the Generate button itself does not change, because there is nothing to wait out.
...
`powerDraw = Σ count × watts`; `powerBudget = POWER_BUDGET_BASE (650 W) + Σ powerBudget effects`; `throttleMult = draw > budget ? 0 : 1` (`src/game/power.ts`): the breaker is a cliff, so past it every rig that needs watts stops and passive income is zero until the draw fits again. Clicking still pays. The store does not sell the card that trips it: `explainBuy` adds a `power` cause (`Trips the breaker · 70 W over budget · install 850 W PSU first`; the step is the unowned PSU on sale or the Graph node on offer, whichever covers the shortfall for the least, from `nextPowerStep`), `canBuy` refuses on it, a `max` buy stops at `unitsWithinBudget`, `saveTarget` skips the unit, and the lock popover's button opens the Power tab on that PSU. Before this rule a card bought one watt too far meant clicking to the next PSU with no income, 12 000 credits of it at the Three-Phase step. What still trips a breaker is the `powerSurge` event or a save from before the rule. Budget comes from the six power upgrades (850 W PSU +300 W for 200 credits up to Fusion Reactor +200 MW for 30 000 000) and the Graph's infra lane (Undervolt +200 W for 5 000 up to Orbital Solar +1 GW for 2e9, plus Antarctic Datacenter +1 MW). Cooling upgrades and nodes add `coolingTier` (+1 speed tier per family). The `powerSurge` event subtracts 40 % of the rig's own budget.

### Player level and XP (`src/game/level.ts`, weights and tables in `src/game/constants.ts`)

**XP is the credits term plus an activity ledger.** The credits term is derived and never stored: `creditsXp = floor(150 × log10(1 + lifetimeCredits))`, 150 XP per decade. Everything else is `stats.xpBy`, a ledger banked per source by `grantXp` the moment the player does the thing, and every grant is announced as an `xp{amount,source}` event so the UI can float it. `playerXp = creditsXp + Σ xpBy`. Both halves survive a rebrand, so a prestige never demotes you, and clicks, the Lounge, hub royalties, level rewards and offline income never grant, so nothing here can be farmed. The other saved field is `stats.levelSeen`, the watermark that makes a level-up reward idempotent across reloads and cloud merges.

| Source | XP | Where it lands |
|---|---|---|
| A landed post | `4 + 3 × model.minLevel`, doubled when viral: SD 1.5 pays 7, FLUX.1 dev 16, a viral FLUX post 32 | `settlePosts`, once per post; a ratioed post pays nothing |
| A contract claimed | 60 | `claimContract` |
| An achievement | 50 | `checkAchievements` |
| A Graph node | 40 | `unlockMapNode` |
| The first unit of a card you have never owned | 50, once per kind whatever the count | `buyHardware`, `grantGift` |
| A named upgrade / a tier upgrade | 40 / 60 | `buyUpgrade` |
| A model set up | 20 | `setupModel` |
| A quantization, new tier only | 30 | `quantize` |
| A LoRA trained | 40 | `trainLora` |
| The daily | `40 × cycle day`, so day 7 pays 280 | `claimDaily`, `claimDailyFromServer` |
| An income milestone | 100 per power of ten of cps | `tick` |
| A rebrand | 500 | `rebrand` |

`xpBreakdown` (which the Stats modal prints row by row) lists `credits earned` first and then every source with something banked, in `XP_SOURCES` order; every row is a whole number, so it always adds up to `playerXp` exactly. A save from before the ledger gets one seeded on load (see the save section above).

`LEVEL_XP` has `MAX_LEVEL = 30` entries, is strictly increasing and starts at 0. Levels 1 to 12 are hand-placed (`LEVEL_XP_HAND`); past them each step is the previous step × `LEVEL_XP_TAIL_GROWTH = 1.1`, rounded to the nearest hundred, so level 13 is 41 400, level 20 is 138 700 and level 30 is 457 200. Retune the hand rungs against the simulator; the tail rule stays.

| L | XP | L | XP | L | XP |
|---|---|---|---|---|---|
| 1 | 0 | 5 | 3 700 | 9 | 14 500 |
| 2 | 700 | 6 | 6 600 | 10 | 18 500 |
| 3 | 1 500 | 7 | 9 000 | 11 | 23 500 |
| 4 | 2 200 | 8 | 11 500 | 12 | 32 000 |

`scripts/balance.ts` models the level with the same constants and functions the engine uses (`LEVEL_XP`, `levelForXp`, `postXp`, `dailyXp`, `levelReward`, every `XP_*` weight, `minLevel` on both tables, the `ACHIEVEMENTS` catalog). Its climber posts at 6/min for the first 3 minutes, 4/min to 10, 2/min to 30 and 1/min after, always on the best model the level opens; banks first-unit, tier, upgrade, Graph-node and achievement XP as it shops; claims one contract per 600 s and one Graph node per 480 s as proxies for the parts of the game it does not play; sets up every checkpoint a new level opens; takes the income milestones and the level reward; and `projectWeek` claims the daily each morning. Every hardware candidate is filtered by `minLevel`, so the level is the pace of the ladder. This is where the hand rungs put each level (`pnpm balance`, `climb` and `naive` over 10 h of continuous play), next to the windows the design asked for:

| L | Window | climb | naive |
|---|---|---|---|
| 2 | 0:30 to 2:30 | 1:27 | 1:38 |
| 3 | 3 to 7 min | 4:46 | 4:46 |
| 4 | 7 to 13 min | 9:39 | 9:16 |
| 5 | 13 to 22 min | 15:02 | 21:45 |
| 6 | 22 to 38 min | 27:02 | 46:05 |
| 7 | 35 to 60 min | 47:35 | 1h 10m |
| 8 | 55 to 90 min | 1h 10m | 1h 36m |
| 9 | 1.5 to 2.6 h | 1h 59m | 2h 24m |
| 10 | 2.4 to 4 h | 3h 08m | 3h 27m |
| 11 | none | 4h 29m | 4h 48m |
| 12 | 5 to 10 h | 7h 04m | 7h 27m |

`pacing.test.ts` asserts the windows as exact bounds on `LEVEL_XP_HAND` and, separately, that every climb arrival stays within 15 % of the seconds behind this table (`MEASURED_ARRIVAL_S`), because the windows sit near the late edge of the current arrivals and a richer XP economy could pull every level 20 % early without failing one. Retuning a weight therefore moves the constants, the test table and this table together; quote nothing about level timing from anywhere else.
...
**What the level gates.** `minLevel` (default 1) is a plain field on both `ModelDef` and `HardwareDef`, not an `UnlockCond`, on purpose: a level-locked model or unit has to stay visible in the store with its reason rather than vanish from the list. For a model, `lockReason` checks it before anything else and `setupModel` refuses with the identical string, `Needs level 4 · you are level 2`; for a unit, `canBuy` refuses with the same words after the family and unlock checks and before the cap and the price, and `explainBuy` lists the cause in that position, so the guidance popover reads the same. `createJob` deliberately does **not** check, so a model that is already installed stays usable if the table moves under it. The Next up panel reads the gate too: `levelBlocked` is true when a unit is already affordable and only the level stands in the way, and `nextGoal` then points at the bar, naming up to three things the next level opens, hardware first; `saveTarget` skips level-locked units, so the save-for bar never asks you to save for something the level will not let you buy. Each level opens one rung of cards and the checkpoints those cards hold natively are never gated below the card.

| L | Hardware | Models |
|---|---|---|
| 1 | `pc-4c8t`, `pc-8c16t` | `sd15` |
| 2 | `mac-mini-m4`, `rx-7600-xt`, `rtx-3060` | `sdxl`, `stable-audio` |
| 3 | `rx-9070-xt`, `rtx-4070-ti-super`, `rtx-3090` | `sd35-medium`, `flux-schnell`, `ace-step` |
| 4 | `rx-7900-xtx`, `rtx-4090`, `rtx-5080`, `mac-studio-m4-max` | `chroma`, `flux-dev`, `hunyuan3d-21` |
| 5 | `rtx-5090`, `radeon-pro-w7900` | `flux-kontext`, `wan22-5b` |
| 6 | `rtx-a6000`, `l4`, `rtx-6000-ada` | `hidream`, `ltx2`, `trellis` |
| 7 | `a40`, `l40s`, `rtx-pro-6000` | `qwen-image`, `qwen-image-edit`, `ideogram`, `recraft`, `grok-imagine` |
| 8 | `a100-80`, `mi300x` | `flux2`, `mochi`, `gpt-image`, `nano-banana`, `flux-pro` |
| 9 | `h100-80`, `mi325x`, `h200` | `cosmos`, `wan22-14b`, `luma-ray`, `vidu`, `hailuo` |
| 10 | `b200`, `b300`, `aws-p4d`, `azure-nd-mi300x` | `hunyuan-video-15`, `kling`, `runway-gen4` |
| 11 | `aws-p5`, `aws-p5e`, `aws-p6`, `runpod-8xb300` | `veo`, `sora` |
| 12 | `region-us-east` | `minimax-h3` |
| 13 | `region-eu-west` | |
| 14 | `region-ap-southeast` | |
| 16 | `orbital-dc` | |
| 20 | `dyson-swarm` | |

`models-data.test.ts` pins the shape rather than the table: every model has an integer `minLevel` in 1..12, `sd15` is 1, every level from 2 to 12 unlocks at least one model so no rung is dead, every API model is 7 or higher, every video model is 5 or higher, and `minLevel` never falls as VRAM rises inside a local kind. `hardware-data.test.ts` pins the hardware side the same way: every unit has an integer `minLevel` in 1..MAX_LEVEL (absent reads as 1), the two CPUs are 1, every level from 2 to 12 opens at least one unit, `minLevel` never falls as `baseCost` rises along the main chain, every `ownHardware` prerequisite sits at or under the unit that needs it, and no quantizable local model is unlocked before a card that holds it natively.

The rest of the level is UI: a `LevelBar` in the hero panel (the percent, the XP to go, up to three chips for what the next level opens, a `+16 XP` float and a pulse on every `xp` event, a fill-and-settle on `levelUp`) with the header `LevelChip` as its compact twin; a `LevelModal` (modal id `level`: the bar, every way to earn XP with live figures, the `xpBreakdown` and the roadmap of every level with its cards, checkpoints and the Lounge) that the bar, the chip, the Next up level goal, the guidance popover, `ModelCard`'s `How do I level up?` and the Stats modal all open; a `LevelUpBanner` that merges two level-ups into `Level 4 to 6` and shows the new cards (`hardware`, the unit ids) over the new checkpoints (`unlocked`, the model ids); confetti plus an electric flash from `FxCanvas`; an `LV N` pill on a level-locked hardware row; and a `level-locked` state on `ModelCard` that beats `api-locked` and `needs-setup`. A level-locked unit shows in the store as locked with `canBuy`'s reason, through the same `nonCreditLock` path as any other store lock.
...
|---|---|---|---|---|
| `nan` | NaN latent | ×0.25 | 0.300 | 0.075 |
| `half` | Half denoised | ×0.5 | 0.393 | 0.1965 |
| `same` | Same seed, same image | ×1 | 0.170 | 0.170 |
| `clean` | Clean sample | ×2 | 0.085 | 0.170 |
| `batch` | Batch of four | ×4 | 0.040 | 0.160 |
| `golden` | Golden seed | ×10 | 0.010 | 0.100 |
| `s42` | Seed 42 | ×42 | 0.002 | 0.084 |
...
`spinEv()` is therefore **0.9555** and `coinEv()` **0.96**, and that is the point. Every seed hands something back: the NaN latent is the dud (the table's lowest multiplier, a quarter of the stake) and it is what the pity meter counts; 13.7 % of bets turn a profit. There is no cooldown and no ceiling on a bet: any amount from `BET_MIN` (10 credits) up to the whole bank, as often as the player likes, with a slider and a number box that always show the same figure. What makes that safe is not a timer, it is the maths, and `gamble.test.ts` pins it by simulating 50 000 bets a table (pity reroll, hot sampler and the minted pot included) and insisting a credit put on either table comes back worth less than a credit. If a future table crosses 1, the Lounge has become an income source and the change is wrong.

On top of that: one free spin per UTC day with a house stake of `max(FREE_SPIN_MIN (200), round(FREE_SPIN_SECS (120) × cps))`, which deducts nothing. A pity meter, where `dryStreak` counts consecutive duds and at `SPIN_PITY_DRY = 3` the next dud is converted once into the cheapest segment above the floor, `half` (the draw still happens, so the rng sequence is identical either way). A hot sampler, where `winStreak` counts consecutive ×2-or-better results and at `SPIN_HOT_STREAK = 3` the widget flips to `control_after_generate: fixed` and payouts carry `SPIN_HOT_MULT = 1.5` until anything under ×2 breaks it (a `nan`, a `half` or even a `same`). And a pot, fed `round(SPIN_POT_FRACTION (0.02) × bet)` by every paid bet on either table, which `s42` takes whole on top of its own payout.

**The documented exception**: a payout moves `state.credits` and nothing else. `lifetimeCredits` feeds XP and therefore the player level, and `seasonCredits` feeds prestige and the leaderboard, so crediting either here would turn the Lounge into an XP farm and let a lucky seed buy a rank. It is the one credit path in the engine that does not go through `addCredits`, it looks like a missing line, and a test pins that both counters stay put. A bet does not call `noteSpend` either: a bank that lands on zero because a coin came up Comfy is not "Out Of Credits, Not Ideas" (the wheel keeps a quarter, so it cannot get there on its own).

Achievements: visible `seed-first` "Random Seed" and `seed-100` "Degenerate Sampler"; hidden `seed-42-hit` "It Was Always 42" (`jackpot42`), `seed-nan-3` "NaN, NaN, NaN" (`nanStreak3`), `seed-hot` "Control After Generate: Fixed" (`hotSeed`), `coin-five` "Call It In The Air" (`coinFive`). `LoungeModal` prints both odds tables above the fold and the footer never hides the edge; the modal throws confetti at ×10 and up and flashes a won flip, both when the result lands rather than when the engine decides it, and only the ×42 raises a toast (held back `SCRAMBLE_MS` for the same reason), because everything smaller is already on the reel in front of you.
...
`nextGoal` picks one thing, first match wins: a done and unclaimed contract (free credits nobody should leave on the table), then a model at or under the player's level that is not set up and whose setup fee is affordable (cheapest fee first, so free ones lead), then the level bar when `levelBlocked` says a unit is already paid for and only the level is in the way, then `saveTarget`, then the cheapest available Graph node already payable in its own currency, then the level bar anyway. It is null only at max level with nothing outstanding. A `level` goal carries the next level, a whole-number percent, `xpToGo` and up to `LEVEL_GOAL_UNLOCKS = 3` names the level opens, hardware first, from `nextUnlocks`. Every credit goal carries a whole-number percent and `etaSec` (`Infinity` at cps 0), so the bar re-renders at most a hundred times per target, and a `buy` goal also carries `unlocksModel`: the biggest model the unit would newly run *natively*, with level-locked models skipped so the hint never promises what the purchase cannot deliver.

`saveTarget` is the cheapest visible, purchasable-but-unaffordable unit, gated by `canBuy`'s non-credit checks in the same order (family, unlock condition, player level, cap, breaker) and priced with `unitCost`, so a level-locked unit, or one the circuit cannot carry, is never the thing you are asked to save for. It moved here out of `storeHooks`, and the hook is now a thin wrapper over it, so the store's save-for bar and the Next up panel cannot quote different numbers; a test pins that they agree over a lived-in save. `isGoalDone` is what flips a row to its done state, and for a `buy` goal it is satisfied when the credits are **banked**, not spent: the goal was to save the price.

### The pacing simulator (`scripts/balance.ts`, `src/game/__tests__/pacing.test.ts`)

`simulate(strategy, seconds)` replays a greedy buyer against the shipped ladder using the game's own `unitCost`, `throttleMult`, `creditsForCp`, `levelForXp`, `postXp`, `dailyXp`, `levelReward` and `crossedMilestones`, with a modelled click schedule (3/s for 3 min, 2/s to 10 min, 1/s to 30 min, then 0.5/s), a post schedule that tapers the same way (6/min, 4/min, 2/min, 1/min) on the best model the level opens, a +30 % content bonus after 3 minutes, global +10 % upgrades at lifetime 1e3…1e9, tier upgrades, and the power ladder (a unit that would trip the breaker is also offered bundled with the PSU steps and infra nodes needed to cover it). It banks XP the way the engine does (first unit, tier, named upgrade, Graph node, achievement, milestone, set-up, the level reward into the bank), proxies one contract per 600 s and one Graph node per 480 s, and filters every hardware candidate by `minLevel`, so each run also records the level it held when it first bought each unit. `naive` buys the best affordable payback every second; `climb` saves for the best payback reachable within 45 s of income. `projectWeek` runs days of one active hour plus 12 offline hours (capped at 8 h × 50 %), claiming the daily each morning. The tests pin: 3060 inside 4 min, 4090 inside 15 min, PRO 6000 inside 60 min, a cloud node inside 4 h (climb); first purchases in ladder order and through the workstation on-ramp; AMD never bought without ROCm; no region inside 3 h of continuous play, the Runpod 8×B300 pod by 8 h and every cloud node by 10; the orbital datacenter and Dyson swarm beyond a one-hour-a-day week while `region-us-east` lands after day 1; every level window in the table under "Player level and XP" and every climb arrival within 15 % of the measured second; naive slower than climb at the 3060, 4090 and 5090 and at levels 5 and 9 over the 10 h runs, and forced to buy power once the breaker trips; and that no run (3 h, 10 h or the fortnight, which reaches levels through the offline settle and the daily too) ever owns a unit whose `minLevel` is above the level it held at purchase. `pnpm balance` prints the tables.
...
| `clickGuard` | The first click the rate cap refuses (`clickGuard.ts`) | Achievement "Rate Limited". |
...
`AchievementToast` also bridges `levelUp`, `milestone`, `weekRollover`, a ratioed `postResolved`, `reward` and the ×42 `spin` (delayed by `SCRAMBLE_MS` so the card lands with the seed) to toasts, so each of those is visible from `/map` and `/hub` too. Every toast it raises passes `sound: false`, because the engine event has already sounded.
...
`sfxMap.ts` is pure and node-testable: the 18 names in `SFX_NAMES` (`achievement, breaker, cash, click, contract, daily, egg, error, flop, jackpot, levelup, lose, purchase, spin, tick, toast, viral, whoosh`), the per-name `BASE_GAIN` and the file-only `SAMPLE_TRIM` (`lose.mp3` and `flop.mp3` are mastered about 13 dB hotter than the rest of the set, so they are pulled down when the file plays and left alone when the recipe does), `cueForEvent(event, ctx)` as an exhaustive switch with a `never` default, `cueForLanding(bet)` for the payoff half of a Lounge bet, `cueForToast(tone)`, `comboRate(combo) = 1 + min(combo, COMBO_RATE_CAP 50) × 0.006` (1.0 alone, 1.3 at a 50-click streak), the `ClickLimiter` token bucket (`CLICK_LIMIT_PER_SEC = 20`), and a synth `Recipe` for every name so an empty `public/sfx` still sounds like a game. The files themselves are 18 mono MP3s under `public/sfx`, 276 KB in total, and a test asserts the folder and the name list stay in step.
...
`useSfx` (mounted once by `GameShell`) subscribes with `useGameEvents` and reads the click streak off the `click` event's `combo`. It also passes `reducedMotion`, which is what silences the Lounge's anticipation cues when a bet lands instantly. The map: `click` → `click` at `comboRate`; `clickBlocked` → `error`; `purchase` → `purchase`, with a second at rate 1.12 after 90 ms for a bulk buy of 10 or more; `upgrade` → `purchase` at 1.12; `mapUnlock` → `whoosh` then `cash`; `jobStarted` → `tick`; `postCreated` → `whoosh`; `postResolved` → `viral` / `lose` on a ratio / `flop` / else `cash`; `achievement` → `egg` for a hidden row, else `achievement`; `levelUp` → `levelup`; `xp` → nothing, because the action that earned it already sounded; `contractDone` → `contract`; `eventStart` → `error`, `breaker`, `egg` or a double tick by kind; `eventEnd` → `cash` only for a fixed node; `daily` → `daily`; `rebrand` → `levelup` at 0.8; `powerThrottle` → `breaker` on, `cash` off; `signup` → `cash` at 1.2; `weekRollover` → double tick; `easterEgg` → `egg`; `milestone` → `levelup` at 0.9; `spin` → `spin` at 1.5 and `flip` → `whoosh` at 1.5, the same cue for a win and a loss, because the reel or the coin is still turning; the payoff (`cueForLanding`: `jackpot` at ×10 and up, `cash` above ×1, `tick` at ×1, `lose` below, `cash` or `lose` for the coin) is played by `LoungeModal` when the seed settles or the face shows, and under reduced motion the event cue is skipped so the landing is the whole sound; `reward` → `cash` then `achievement`. `offline` is deliberately silent, because it fires before any gesture has unlocked audio. Toasts play `cueForToast(tone)` when a card mounts unless they pass `sound: false`, which every toast raised from an engine event does.
...
- **The click guard's window is per `GameState` object.** The counted clicks live in a `WeakMap` keyed on the state, so `replaceState` (an import, a hard reset, adopting a cloud save) starts it empty. That is the right behaviour for a fresh run, and the most it can buy anyone is one extra second at the cap.
