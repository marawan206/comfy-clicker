...
export function GenerateButton() {
  ...
  /** Feedback for a click at a viewport point, whatever produced it. */
  const feedback = useCallback(
    (clientX: number, clientY: number, value: number) => {
      squash()
  ...
  const clickAt = useCallback(
    (clientX: number, clientY: number) => {
      ...
      feedback(clientX, clientY, value)
    },
    [store, feedback],
  )
  ...
  useGameEvents((event) => {
    ...
    const rect = buttonRef.current?.getBoundingClientRect()
    if (!rect) return
    feedback(rect.left + rect.width / 2, rect.top + rect.height * 0.42, event.value)
  })
    ...
    <div className="flex w-full flex-col items-center gap-3">
      <div className="relative flex items-center justify-center" style={{ width: BOX, height: BOX, maxWidth: '100%' }}>
        ...
        <motion.button
          ...
          className="relative isolate flex cursor-pointer select-none items-center justify-center overflow-hidden border-2 border-charcoal-300 bg-charcoal-700 shadow-[0_6px_0_#0e0e0f] outline-none [-webkit-tap-highlight-color:transparent] focus-visible:ring-4 focus-visible:ring-electric-400"
        >
          <span
            aria-hidden="true"
      ...
      <motion.button
        ...
        className="inline-flex cursor-pointer select-none items-center gap-2 rounded-full bg-electric-400 px-6 py-2.5 text-base font-bold text-charcoal-800 shadow-[0_4px_0_#0e0e0f] outline-none [-webkit-tap-highlight-color:transparent] focus-visible:ring-4 focus-visible:ring-electric-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-charcoal-600"
      >
        <Play size={18} strokeWidth={3} fill="currentColor" aria-hidden="true" />
        Generate
      </motion.button>

      <p className="flex items-center gap-1 text-sm font-semibold tabular-nums text-smoke-600">
        <span className="text-credits">+{formatClickValue(clickValue)}</span>
        <CreditsIcon size={13} className="text-credits" />
        <span>per click</span>
      </p>
    </div>
  )
