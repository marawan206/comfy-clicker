    ...
    <div className={cn("relative", className)} {...props}>
      <DropdownMenu onOpenChange={setIsOpen}>
        <div className="group relative">
          <DropdownMenuTrigger render={<button className="flex items-center gap-16 rounded-2xl border border-zinc-200/60 bg-white p-3 transition-all duration-200 hover:border-zinc-300 hover:bg-zinc-50/80 hover:shadow-sm focus:outline-none dark:border-zinc-800/60 dark:bg-zinc-900 dark:hover:border-zinc-700 dark:hover:bg-zinc-800/40" type="button" />}><div className="flex-1 text-left">
          ...
          >
            <div className="space-y-1">
              {menuItems.map((item) => (
                <DropdownMenuItem key={item.label} render={<Link className="group flex cursor-pointer items-center rounded-xl border border-transparent p-3 transition-all duration-200 hover:border-zinc-200/50 hover:bg-zinc-100/80 hover:shadow-sm dark:hover:border-zinc-700/50 dark:hover:bg-zinc-800/60" href={item.href} />}><div className="flex flex-1 items-center gap-2">
                                        {item.icon}
                                      ...
                                      </div><div className="ml-auto flex-shrink-0">
                                        {item.value && (
                                          <span
                                            className={cn(
            ...
            <DropdownMenuItem render={<button className="group flex w-full cursor-pointer items-center gap-3 rounded-xl border border-transparent bg-red-500/10 p-3 transition-all duration-200 hover:border-red-500/30 hover:bg-red-500/20 hover:shadow-sm" type="button" />}><LogOut className="h-4 w-4 text-red-500 group-hover:text-red-600" /><span className="font-medium text-red-500 text-sm group-hover:text-red-600">
                                        Sign Out
